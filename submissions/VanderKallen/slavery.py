...
_Constants._DATABASE_NAME = "slavery.db"
...
def get_transaction(test=True):
    ...
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM slavery LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM slavery".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
