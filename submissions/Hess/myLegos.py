...
class ClawBot:
    ...
    def reset(self):
        ...
        while(basetouch == False):
            ClawBot.baseMotor.run_forever(speep_sp = 360)
            time.sleep(1)
            basetouch = ClawBot.baseTouch.is_pressed()
        while(armtouch == False):
            ClawBot.armMotor.run_forever(speed_sp = -360)
            time.sleep(1)
            armtouch = ClawBot.armTouch.is_pressed()
        return 0