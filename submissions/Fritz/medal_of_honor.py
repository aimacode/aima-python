...
class _Constants(object):
    ...
    '''
    _HEADER = {'User-Agent': 
              'CORGIS Medal Of Honor library for educational purposes'}
...
_Constants._DATABASE_NAME = "medal_of_honor.db"
...
def get_awardees(test=True):
    """
    Returns a list of the awardees in the database.
    
    """
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM medal_of_honor LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM medal_of_honor".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
...
def _test_interfaces():
    ...
    # Production test
    print("Production get_awardees")
    start_time = _default_timer()
    result = get_awardees(test=False)
