...
wine_norm = min_max_scaler.fit_transform(wineData)
...
def main():
    ...
    classes = {0: [],
               1: [],
               2: []}
    for i in range(len(wine_norm)):
        alcohol = wine_norm[i][0]
        hue = wine_norm[i][10]
        wine = [alcohol, hue]
        classes[labels[i]].append(wine)
    for i in range(0, 3):
        classes[i] = np.array(classes[i], dtype='float64')
    plt.xlabel('Alcohol')
    plt.ylabel('Hue')
    plt.scatter(classes[0][:, 0], classes[0][:, 1], c='r')
    plt.scatter(classes[1][:, 0], classes[1][:, 1], c='b')
