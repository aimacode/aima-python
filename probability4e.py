...
def is_independent(variables, P):
    """Return whether a list of variables are independent given their
    distribution P"""
    for var in variables:
        event = copy.copy(variables).remove(var)
...
def naive_bayes_ask(X, e, P):
    """X: cause, e: events, P: joint distribution
    formula 12.21"""
    Q = ProbDist(X)
    for xi in P.values(X):
        Q[xi] = P[xi]
        for e,val in enumerate(e):
            new_e = {X:xi, e:val}
            Q[xi] *= P[new_e]
    return Q.normalize()
