...
def test_best_first_graph_search():
    ...
    assert best_first_graph_search(
        romania_problem,
        lambda node: node.state)[0] == ['Sibiu', 'Fagaras', 'Bucharest']
    assert best_first_graph_search(
        romania_problem,
        lambda node: node.state[::-1])[0] == ['Timisoara',
                                                      'Lugoj',
                                                      'Mehadia',
                                                      'Drobeta',
                                                      'Craiova',
...
def test_uniform_cost_search():
    assert uniform_cost_search(
        romania_problem)[0] == ['Sibiu', 'Rimnicu', 'Pitesti', 'Bucharest']
    assert uniform_cost_search(nqueens)[0] == [0, 4, 7, 5, 2, 6, 1, 3]


def test_depth_first_tree_search():
    assert depth_first_tree_search(nqueens) == [7, 3, 0, 2, 5, 1, 6, 4]
...
def test_astar_search():
    assert astar_search(romania_problem)[0] == ['Sibiu', 'Rimnicu', 'Pitesti', 'Bucharest']
    assert astar_search(eight_puzzle)[0] == ['LEFT', 'LEFT', 'UP', 'RIGHT', 'RIGHT', 'DOWN', 'LEFT', 'UP', 'LEFT', 'DOWN', 'RIGHT', 'RIGHT']
    assert astar_search(EightPuzzle((1, 2, 3, 4, 5, 6, 0, 7, 8)))[0] == ['RIGHT', 'RIGHT']
    assert astar_search(nqueens)[0] == [7, 1, 3, 0, 6, 4, 2, 5]
