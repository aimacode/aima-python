...
# Problems
prob_1 = Problem('At(Home) & Have(Cash) & Have(Car) ', 'At(SFO) & Have(Cash)',
                 [go_SFO, taxi_SFO, drive_SFOLongTermParking, shuttle_SFO])

initialPlan = [AngelicNode(prob_1.init, None, [angelic_opt_description], [angelic_pes_description])]


def test_refinements():
    result = [i for i in Problem.refinements(go_SFO, library_1)]
...
def test_optimistic_reachable_set():
    ...
    plan = AngelicNode(problem.init, None, [h_1, h_2], [h_1, h_2])
...
def test_making_progress():
    ...
    plan_1 = AngelicNode(prob_1.init, None, [angelic_opt_description], [angelic_pes_description])
